...
/**
 * SentiTradePulse - Fully responsive, dynamic-path version.
 * @param {number} value - Sentiment (0-100)
 * @param {string} assetName - Asset name
 */
const SentiTradePulse = ({ value, assetName }) => {
  ...
  const dustRotation = value * 1.8 - 90;
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-6 shadow-inner">
      ...
      >
        ...
        {/* 4. --- Vertical Text (Bearish/Bullish) --- */}
        <div
        ...
        {/* Needle */}
        <div
          className="absolute z-30 transition-transform duration-1000 ease-out origin-bottom shadow-[0_0_15px_white]"
          style={{
            bottom: "0px",
            left: `${cx - 2}px`,
            width: "4px",
        ...
        >
          <div className="absolute top-[-80px] w-1.5 h-1.5 rounded-full bg-cyan-300 shadow-[0_0_10px_#22d3ee]" />
        </div>
      </div>
      ...
      <div className="relative z-10 text-center mt-3">
        <p
          className={`text-xl font-bold ${value < 40 ? "text-red-500" : value < 60 ? "text-zinc-200" : "text-green-500"} uppercase tracking-wider`}
        >
          {value < 40 ? "Bearish" : value < 60 ? "Neutral" : "Bullish"}
        </p>
